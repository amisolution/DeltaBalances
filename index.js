{
	...
	var exchanges =
		{
			...
			'SingularX': {
				...
				loaded: 0,
				displayed: 0,
				contract: _delta.config.contractSingularxAddr
			},
			'Etherc': {
				enabled: false,
				loaded: 0,
				displayed: 0,
				contract: _delta.config.contractEthercAddr
			},
		};
	...
	function buildHtmlTable(selector, myList, loaded, headers) {
		...
		for (var i = 0; i < myList.length; i++) {
			...
			for (var colIndex = 0; colIndex < columns.length; colIndex++) {
				...
				if (head == 'Total' || head == 'EtherDelta' || head == 'Singularx' || head == 'Decentrex' || head == 'Token store' || head == 'IDEX' || head == 'Enclaves' || head == 'DEXY' || head == 'Ethen' || head == 'Wallet' || head == 'Bid' || head == 'Ask' || head == 'Est. ETH') {
	...
	var balanceHeaders = { 'Name': 1, 'Wallet': 1, 'EtherDelta': 1, 'IDEX': 1, 'Token store': 1, 'Enclaves': 1, 'Decentrex': 1, 'DEXY': 1, 'Ethen': 1, 'Singularx': 1, 'Etherc': 1, 'Total': 1, 'Value': 1, 'Bid': 1, 'Ask': 0, 'Est. ETH': 1, 'USD': 0, 'EUR': 0 };
		...
		if (lastResult) {
			...
			for (var i = 0; i < allBal.length; ++i) {
				...
				for (let j = 0; j < arr.length; j++) {
					...
					if (A[0][j] == 'Wallet' || A[0][j] == 'EtherDelta' || A[0][j] == 'Etherc' || A[0][j] == 'Singularx' || A[0][j] == 'IDEX' || A[0][j] == 'Token store' || A[0][j] == 'Enclaves' || A[0][j] == 'Decentrex' || A[0][j] == 'DEXY' || A[0][j] == 'Ethen' || A[0][j] == 'Total' || A[0][j] == 'Estimated value (ETH)' || A[0][j] == 'Bid (ETH)' || A[0][j] == 'Ask (ETH)') {
						if (arr[j] != '' && arr[j] != ' ')
